...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('vendor.menu.store') }}" method="POST" enctype="multipart/form-data" class="forms-sample" id="form">
                @csrf
                <div class="form-group">
                    ...
                    <input type="text" required name="nama" class="form-control form-control-sm mb-3" id="namaMenu"
                        maxlength="30" minlength="3" placeholder="ex. Es Teler">
                    @error('namaMenu')
                        <p class="text-sm text-red-600 mt-1.5">{{ $message }}</p>
                    @enderror
