...
@section('title', 'Add Penjualan (Axios)')
@php
    $breadcrumbs = [
        'Penjualan' => route('admin.barang'),
        'Add Penjualan' => null
    ];
@endphp
@push('styles')
                ...
                <div class="modal-body">
                    <form action="#" id="modal-form">
                        <div class="form-group">
                            ...
                            <input type="text" name="id_barang" id="editIdBarang" class="form-control mb-2" required readonly>
                            <label for="nama">Nama Barang</label>
                            <input type="text" name="nama" id="editNamaBarang" class="form-control mb-2" required readonly>
                            <label for="harga">Harga Barang</label>
                            <input type="number" name="harga" id="editHargaBarang" class="form-control mb-2" required readonly>
                            <label for="jumlah">Jumlah</label>
                            <input type="number" name="jumlah" id="editJumlah" class="form-control mb-2" required>
                            <label for="subtotal">Subtotal</label>
                            <input type="number" name="subtotal" id="editSubtotal" class="form-control mb-2" required readonly>
                        </div>
                    </form>
                </div>
...
@push('scripts')
    ...
    <script>
        ...
        $(function () {
            ...
            // ── Fetch barang berdasarkan kode (Axios GET) ──────────────────
            $('#idBarang').on('input', function () {
                let kode = $('#idBarang').val();

                axios.get('/admin/penjualan/barang/' + kode)
                    .then(function (response) {
                        // Axios: response JSON ada di response.data
                        // sehingga data barang ada di response.data.data.barang
                        $('#namaBarang').val(response.data.data.barang?.nama ?? '');
                        $('#hargaBarang').val(response.data.data.barang?.harga ?? 0);
                        $('#jumlah').val(1);
                    })
            ...
            addButton.click(function () {
                ...
                // Cek duplikat kode di tabel
                $("#table tbody tr").each(function () {
                    previousKode = $(this).find("td:eq(0)").text();

                    if (previousKode == kode) {
                        let updateJumlah   = parseInt($(this).find("td:eq(3)").text()) + jumlah;
                        let updateSubtotal = parseInt($(this).find("td:eq(4)").text()) + subtotal;
            ...
            // ── Klik baris tabel → buka modal edit ────────────────────────
            ...
            $('#editJumlah').on('input', function () {
                let harga    = parseInt($('#editHargaBarang').val());
            ...
            $("#submitBtn").click(function () {
                ...
                submitBtn.html('<span class="spinner-border spinner-border-sm"></span> Loading');

                // Axios POST — CSRF token sudah di-set lewat axios.defaults di atas
                axios.post('/admin/penjualan/store/', {
                    items: items,
                    total: total,
                })
                .then(function (response) {
                    Swal.fire('Success', 'Penjualan added', 'success');
